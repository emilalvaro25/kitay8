...
import PopUp from './components/demo/popup/PopUp';
...
function App() {
  ...
  const { theme, setAudioChimes, setSoundEffects, presentationData, isCameraOn, viewingApp, isAddingApp, setIsAddingApp } = useUI();
  ...
  const handleClosePopup = () => {
    ...
    // Play a sound on user interaction to unlock audio context
    clickSoundRef.current?.play().catch(e => console.warn("Audio context unlock attempt.", e));
  };
  ...
  return (
    <LiveAPIProvider apiKey={API_KEY} user={user}>
      {showWelcomePopup && <PopUp onClose={handleClosePopup} />}
